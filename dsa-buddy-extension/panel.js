...
chrome.tabs.onUpdated.addListener((tabId, info, tab) => {
  if (info.status === "complete") {
    init(); // re-fetch problem after reload
  }
});
...
function init() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    if (
      activeTab.url &&
      (activeTab.url.includes("leetcode.com") ||
        activeTab.url.includes("codeforces.com"))
    ) {
...
function displayHints(hints) {
  ...
  const renderSnippet = (code, title) => {
    ...
    html = html.replace(/"(.*?)"|'(.*?)'/g, (match) =>
      saveToken("string", match),
    );
    html = html.replace(/\/\/.*/g, (match) => saveToken("comment", match));

    const keywords =
      /\b(public|private|static|void|int|char|boolean|new|class|return|if|else|for|while|do|switch|case|break|continue|try|catch|finally|throw|throws|extends|implements|this|super|null|true|false|const|let|var|function|async|await|import|from)\b/g;
...
function showError(msg) {
  ...
  if (msg.includes("communicate")) {
    ...
    btn.style.boxShadow = "0 2px 6px rgba(154, 187, 25, 0.84)";
