...
app.post("/hints", async (req, res) => {
  try {
    ...
    if (mode === "logic") {
      prompt = `
          ...
          Response Format (JSON):
          {
            ...
            "hint3": "**Optimized Logic**: [Strong hint on the correct path] <=70 words",
            "snippet3": "Make sure no comments and less than 12 lines [Code block for Hint 3] plain code only, max 12 lines, no comments""
          }
        `;
      ...
      prompt = `
          ...
          Response Format (JSON):
          {
            ...
            "hint3": "Template snippet explanation <=70 words",
            "snippet3": "plain code only, max 12 lines, no comments"
    ...
    try {
      ...
      try {
        ...
        return res.status(500).json({
          error:
            "Failed to generate valid hints. The AI returned invalid data.",
        });
      }
    }
    ...
    if (error.message.includes("429")) {
      return res
        .status(429)
        .json({ error: "AI Rate Limit Exceeded. Please wait a moment." });
    }
